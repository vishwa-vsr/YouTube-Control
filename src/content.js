...
} from './modules/comment-canvas.js';

import {
  initSpeedController,
  onSettingsUpdate as updateSpeedSettings,
  injectSpeedButton,
  enforcePersistedPlaybackRate,
  getActiveVideoElement
} from './modules/speed-controller.js';
...
} from './modules/watch-navigation.js';
...
function runContentScript() {
  ...
  function applySettings(settings) {
    ...
    // Apply configurations via schema state machine
    const { classesToAdd, classesToRemove } = resolveActiveClasses(settings, isEnabled);
    classesToAdd.forEach(cls => root.classList.add(cls));
  ...
  function injectCommentScreenshotButtons() {
    ...
    commentElements.forEach(commentEl => {
      ...
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        e.preventDefault();
        captureCommentToClipboard(commentEl, btn);
      }, true);
    ...
    try {
      chrome.runtime.onMessage.addListener((message) => {
        if (message.action === 'updateSettings') {
          cachedSettings = getDefaultSettings(message.settings || {});
          updateSpeedSettings(cachedSettings);
          applySettings(cachedSettings);
        }
      });
    } catch (err) {}
  ...
  // Watch navigation & dual-scroll coordination
  initWatchNavigation({
    isTopWindow,
    onNavigateStart: ({ isDifferentVideo }) => {
      if (isDifferentVideo) {

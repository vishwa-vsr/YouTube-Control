...
### Fixed
- **Search Bar Centering**: Fixed an issue where turning on the scroll distance tracker shifted YouTube's search bar off-center.
...
### Changed
- **Codebase Architecture Overhaul**: Restructured monolithic content scripts into self-contained, independently testable ES modules with automated `esbuild` bundling in `build.py`.
